...
func loadInitSchema() (string, error) {
	initSchemaOnce.Do(func() {
		...
		if !ok {
			initSchemaErr = fmt.Errorf("unable to resolve caller path for migrations")
			return
		}
		projectRoot := filepath.Clean(filepath.Join(filepath.Dir(currentFile), "..", "..", ".."))
		path := filepath.Join(projectRoot, "db", "migrations", "000001_init_realtime.sql")
...
// ApplyAll executes the baseline SQL migrations.
